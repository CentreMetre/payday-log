...
docker exec -i payday-log-dev pg_restore -U postgres -d "$DATABASE" --no-owner < "$HOME/Backup/psql/Payday-backup-latest.dump"
...
docker exec -i payday-log-dev psql -U postgres -d "$DATABASE" < "create-new-db-schema.sql"
docker exec -i payday-log-dev psql -U postgres -d "$DATABASE" < "populate-new-schema.sql"
